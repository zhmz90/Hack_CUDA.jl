...
__device__ void vec_select_sort(float *d_A, int left, int right, int depth)
{


}

__global__ void vec_quick_sort(float *d_A, int len)
{
...
int main(void)
{
  ...
  //  printf("cuda call stack heigth is %d\n", (int)cudaDeviceGetLimit(cudaLimitDevRuntimeSyncDepth));
    ...
    vec_quick_sort<<<dimGrid,dimBlock,0,0>>>(d_A,len);
//    cudaDeviceSynchronize();
